...
def _make_task(name, sp, target_date, is_overdue=False, orig_date=None, help_fields=None):
    return {
        ...
        "display_name": name.upper() if is_overdue else name,
        "offset_days": 5,
        "help_key": None,
        "help_label": "",
...
class TestTodaysToDo:
    ...
    def test_task_due_today_appears_in_tasks_table(self):
        task = _make_task("FILE RESPONSE WITH USPTO", "A.1", TODAY, is_overdue=True)
    ...
    def test_no_tasks_today_shows_placeholder(self):
        task = _make_task("Future task", "A.1", TODAY + timedelta(days=5))
        entry = _base_entry(_tasks=[task])
        md = build_markdown([entry], TODAY, CSV_META)
...
class TestPrioritySection:
    def test_priority_entry_appears(self):
        entry = _base_entry(_priority_score=100, _priority_letter="A")
        md = build_markdown([entry], TODAY, CSV_META)
    ...
    def test_entry_shows_metadata(self):
        ...
        assert "FAM-TEST" in md
        assert "17/999,999" in md

    def test_task_block_rendered(self):
        task = _make_task("Review Office Action", "A.1", TODAY + timedelta(days=10))
        entry = _base_entry(_tasks=[task])
        md = build_markdown([entry], TODAY, CSV_META)
        assert "A.1" in md
    ...
    def test_overdue_task_block_shows_warning(self):
        task = _make_task(
            "Review Office Action", "A.1", TODAY,
            is_overdue=True, orig_date=date(2024, 10, 1)
        )
        entry = _base_entry(_tasks=[task])
        md = build_markdown([entry], TODAY, CSV_META)
        assert "OVERDUE" in md
        assert "Originally due 2024-10-01" in md

    def test_help_fields_rendered(self):
        task = _make_task(
            "Consult Inventor", "A.2", TODAY + timedelta(days=5),
    ...
    def test_below_threshold_entry_not_in_priority_section(self):
        low_entry = _base_entry(_priority_score=20, _priority_letter="B", _tasks=[])
        md = build_markdown([low_entry], TODAY, CSV_META)
        # Should NOT appear under Priority Docket Entries with a task block
        assert "[B]" not in md.split("Priority Docket Entries")[1].split("Monitored")[0]
